...
namespace ACE.Server.DotRecast.Detour.Dynamic
{
    /**
 ...
 */
     ...
     */
        ...
        private bool TraverseTiles(RcVec3f start, RcVec3f end, out float hit)
        {
            ...
            int sx = (int)(float)Math.Floor(relStartX / tileWidth);
            int sz = (int)(float)Math.Floor(relStartZ / tileDepth);
            int ex = (int)(float)Math.Floor((end.X - origin.X) / tileWidth);
            int ez = (int)(float)Math.Floor((end.Z - origin.Z) / tileDepth);
            int dx = ex - sx;
            int dz = ez - sz;
            int stepX = dx < 0 ? -1 : 1;
            int stepZ = dz < 0 ? -1 : 1;
            ...
            float xOffest = (float)Math.Abs(tx < 0 ? xRem : tileWidth - xRem);
            float zOffest = (float)Math.Abs(tz < 0 ? zRem : tileDepth - zRem);
            tx = (float)Math.Abs(tx);
            tz = (float)Math.Abs(tz);
        ...
        private bool TraversHeightfield(int x, int z, RcVec3f start, RcVec3f end, float tMin, float tMax, out float hit)
        {
            ...
            if (null != hf)
            {
                ...
                int sx = (int)(float)Math.Floor(relStartX / hf.cs);
                int sz = (int)(float)Math.Floor(relStartZ / hf.cs);
                int ex = (int)(float)Math.Floor((exit[0] - hf.bmin.X) / hf.cs);
                int ez = (int)(float)Math.Floor((exit[2] - hf.bmin.Z) / hf.cs);
                int dx = ex - sx;
                int dz = ez - sz;
                int stepX = dx < 0 ? -1 : 1;
                int stepZ = dz < 0 ? -1 : 1;
                ...
                float xOffest = (float)Math.Abs(tx < 0 ? xRem : hf.cs - xRem);
                float zOffest = (float)Math.Abs(tz < 0 ? zRem : hf.cs - zRem);
                tx = (float)Math.Abs(tx);
                tz = (float)Math.Abs(tz);
                float tMaxX = xOffest / tx;
                float tMaxZ = zOffest / tz;
