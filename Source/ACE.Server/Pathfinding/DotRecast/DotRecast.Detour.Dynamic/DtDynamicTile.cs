...
namespace ACE.Server.DotRecast.Detour.Dynamic
{
    public class DtDynamicTile
    {
        ...
        private RcHeightfield BuildHeightfield(DtDynamicNavMeshConfig config, RcContext context)
        {
            ...
            {
                if (!rasterizedColliders.Contains(kv.Key))
                {
                    heightfield.bmax.Y = Math.Max(heightfield.bmax.Y, kv.Value.Bounds()[4] + heightfield.ch * 2);
                    kv.Value.Rasterize(heightfield, context);
                }
            }
