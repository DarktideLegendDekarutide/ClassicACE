...
using ACE.Server.DotRecast.Detour.Dynamic.Io;
...
namespace ACE.Server.DotRecast.Detour.Dynamic
{
     ...
     */
        ...
        private ICollection<DtDynamicTile> GetTiles(float[] bounds)
        {
            ...
            int minx = (int)(float)Math.Floor((bounds[0] - navMeshParams.orig.X) / navMeshParams.tileWidth);
            int minz = (int)(float)Math.Floor((bounds[2] - navMeshParams.orig.Z) / navMeshParams.tileHeight);
            int maxx = (int)(float)Math.Floor((bounds[3] - navMeshParams.orig.X) / navMeshParams.tileWidth);
            int maxz = (int)(float)Math.Floor((bounds[5] - navMeshParams.orig.Z) / navMeshParams.tileHeight);
