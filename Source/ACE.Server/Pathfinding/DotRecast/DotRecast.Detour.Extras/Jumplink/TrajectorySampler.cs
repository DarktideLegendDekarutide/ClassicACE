...
namespace ACE.Server.DotRecast.Detour.Extras.Jumplink
{
    class TrajectorySampler
    {
        ...
        private bool SampleTrajectory(JumpLinkBuilderConfig acfg, RcHeightfield solid, RcVec3f pa, RcVec3f pb, Trajectory tra)
        {
            ...
            float d = RcVecUtils.Dist2D(pa, pb) + (float)Math.Abs(pa.Y - pb.Y);
            int nsamples = Math.Max(2, (int)(float)Math.Ceiling(d / cs));
            for (int i = 0; i < nsamples; ++i)
            {
        ...
        private bool CheckHeightfieldCollision(RcHeightfield solid, float x, float ymin, float ymax, float z)
        {
            ...
            int ix = (int)(float)Math.Floor((x - orig.X) / cs);
            int iz = (int)(float)Math.Floor((z - orig.Z) / cs);
