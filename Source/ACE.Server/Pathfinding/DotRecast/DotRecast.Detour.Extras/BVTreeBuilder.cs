...
namespace ACE.Server.DotRecast.Detour.Extras
{
    public class BVTreeBuilder
    {
        ...
        private static int CreateBVTree(DtMeshData data, DtBVNode[] nodes, float quantFactor)
        {
            ...
            for (int i = 0; i < data.header.polyCount; i++)
            {
                ...
                it.bmin[0] = Extensions.MathClamp((int)((bmin.X - data.header.bmin.X) * quantFactor), 0, 0x7fffffff);
                it.bmin[1] = Extensions.MathClamp((int)((bmin.Y - data.header.bmin.Y) * quantFactor), 0, 0x7fffffff);
                it.bmin[2] = Extensions.MathClamp((int)((bmin.Z - data.header.bmin.Z) * quantFactor), 0, 0x7fffffff);
                it.bmax[0] = Extensions.MathClamp((int)((bmax.X - data.header.bmin.X) * quantFactor), 0, 0x7fffffff);
                it.bmax[1] = Extensions.MathClamp((int)((bmax.Y - data.header.bmin.Y) * quantFactor), 0, 0x7fffffff);
                it.bmax[2] = Extensions.MathClamp((int)((bmax.Z - data.header.bmin.Z) * quantFactor), 0, 0x7fffffff);
            }
