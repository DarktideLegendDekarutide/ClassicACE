        ...
        private static int CreateBVTree(DtNavMeshCreateParams option, DtBVNode[] nodes)
        {
            ...
            for (int i = 0; i < option.polyCount; i++)
            {
                ...
                if (option.detailMeshes != null)
                {
                    ...
                    it.bmin[0] = Extensions.MathClamp((int)((bmin.X - option.bmin.X) * quantFactor), 0, int.MaxValue);
                    it.bmin[1] = Extensions.MathClamp((int)((bmin.Y - option.bmin.Y) * quantFactor), 0, int.MaxValue);
                    it.bmin[2] = Extensions.MathClamp((int)((bmin.Z - option.bmin.Z) * quantFactor), 0, int.MaxValue);

                    it.bmax[0] = Extensions.MathClamp((int)((bmax.X - option.bmin.X) * quantFactor), 0, int.MaxValue);
                    it.bmax[1] = Extensions.MathClamp((int)((bmax.Y - option.bmin.Y) * quantFactor), 0, int.MaxValue);
                    it.bmax[2] = Extensions.MathClamp((int)((bmax.Z - option.bmin.Z) * quantFactor), 0, int.MaxValue);
                }
                else
                {
                    ...
                    // Remap y
                    it.bmin[1] = (int)(float)Math.Floor(it.bmin[1] * option.ch * quantFactor);
                    it.bmax[1] = (int)(float)Math.Ceiling(it.bmax[1] * option.ch * quantFactor);
                }
            }
