...
namespace ACE.Server.DotRecast.Detour
{
     ...
     */
        ...
        {
            tx = (int)(float)Math.Floor((pos.X - m_orig.X) / m_tileWidth);
            ty = (int)(float)Math.Floor((pos.Z - m_orig.Z) / m_tileHeight);
        }
        ...
        List<long> QueryPolygonsInTile(DtMeshTile tile, RcVec3f qmin, RcVec3f qmax)
        {
            ...
            if (tile.data.bvTree != null)
            {
                ...
                float minx = Extensions.MathClamp(qmin.X, tbmin.X, tbmax.X) - tbmin.X;
                float miny = Extensions.MathClamp(qmin.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                float minz = Extensions.MathClamp(qmin.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                float maxx = Extensions.MathClamp(qmax.X, tbmin.X, tbmax.X) - tbmin.X;
                float maxy = Extensions.MathClamp(qmax.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                float maxz = Extensions.MathClamp(qmax.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                // Quantize
        ...
        public long AddTile(DtMeshData data, int flags, long lastRef)
        {
            ...
            Extensions.ArrayFill(tile.polyLinks, DtNavMesh.DT_NULL_LINK);
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    foreach (var connectPoly in connectPolys)
                    {
                        ...
                        if (dir == 0 || dir == 4)
                        {
                            ...
                            link.bmin = (int)(float)Math.Round(Extensions.MathClamp(tmin, 0.0f, 1.0f) * 255.0f);
                            link.bmax = (int)(float)Math.Round(Extensions.MathClamp(tmax, 0.0f, 1.0f) * 255.0f);
                        }
                        else if (dir == 2 || dir == 6)
                        {
            ...
            ref List<DtConnectPoly> cons)
        {
            ...
            for (int i = 0; i < tile.data.header.polyCount; ++i)
            {
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    if ((float)Math.Abs(apos - bpos) > 0.01f)
                    {
                        continue;
                    }
