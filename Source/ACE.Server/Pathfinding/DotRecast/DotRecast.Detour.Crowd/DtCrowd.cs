...
using Extensions = ACE.Server.DotRecast.Core.Extensions;
     ...
     */
        ...
        private int GetNeighbours(RcVec3f pos, float height, float range, DtCrowdAgent skip, ref List<DtCrowdNeighbour> result, DtProximityGrid grid)
        {
            ...
            for (int i = 0; i < nids; ++i)
            {
                ...
                if ((float)Math.Abs(diff.Y) >= (height + ag.option.height) / 2.0f)
                {
                    continue;
                }
        ...
        {
            return Extensions.MathClamp((t - t0) / (t1 - t0), 0.0f, 1.0f);
        }
    }
}