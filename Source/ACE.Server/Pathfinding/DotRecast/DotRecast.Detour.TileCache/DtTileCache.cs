...
namespace ACE.Server.DotRecast.Detour.TileCache
{
    public class DtTileCache
    {
        ...
        private List<long> QueryTiles(RcVec3f bmin, RcVec3f bmax)
        {
            ...
            int tx0 = (int)(float)Math.Floor((bmin.X - m_params.orig.X) / tw);
            int tx1 = (int)(float)Math.Floor((bmax.X - m_params.orig.X) / tw);
            int ty0 = (int)(float)Math.Floor((bmin.Z - m_params.orig.Z) / th);
            int ty1 = (int)(float)Math.Floor((bmax.Z - m_params.orig.Z) / th);
