...
using ACE.Server.DotRecast.Detour.TileCache.Io;
...
namespace ACE.Server.DotRecast.Detour.TileCache
{
    public class DtTileCacheBuilder
    {
        ...
        public void BuildTileCacheRegions(DtTileCacheLayer layer, int walkableClimb)
        {
            ...
            Extensions.ArrayFill(layer.regs, (short)0x00FF);
            int nsweeps = w;
            ...
            for (int y = 0; y < h; ++y)
            {
                if (regId > 0)
                {
                    Extensions.ArrayFill(prevCount, 0, 0, regId);
                }
        ...
        bool IsConnected(DtTileCacheLayer layer, int ia, int ib, int walkableClimb)
        {
            if (layer.areas[ia] != layer.areas[ib])
                return false;
            if ((float)Math.Abs(layer.heights[ia] - layer.heights[ib]) > walkableClimb)
                return false;
            return true;
        }
            ...
            for (int dz = -1; dz <= 0; ++dz)
            {
                for (int dx = -1; dx <= 0; ++dx)
                {
                    ...
                    if (px >= 0 && pz >= 0 && px < w && pz < h)
                    {
                        ...
                        if ((float)Math.Abs(lh - y) <= walkableClimb && layer.areas[idx] != DT_TILECACHE_NULL_AREA)
                        {
        ...
        private int AddVertex(int x, int y, int z, int[] verts, int[] firstVert, int[] nextVert, int nv)
        {
            ...
            {
                int tv = i * 3;
                if (verts[tv] == x && verts[tv + 2] == z && ((float)Math.Abs(verts[tv + 1] - y) <= 2))
                    return i;
                i = nextVert[i]; // next
            }
        ...
        private void MergePolys(int[] polys, int pa, int pb, int ea, int eb, int maxVertsPerPoly)
        {
            ...
            // Merge polygons.
            Extensions.ArrayFill(tmp, DT_TILECACHE_NULL_IDX);
            int n = 0;
            // Add pa
        ...
        private void RemoveVertex(DtTileCachePolyMesh mesh, int rem, int maxTris)
        {
            ...
            for (int i = 0; i < mesh.npolys; ++i)
            {
                ...
                if (hasRem)
                {
                    ...
                    Extensions.ArrayFill(mesh.polys, DT_TILECACHE_NULL_IDX, p + maxVertsPerPoly, maxVertsPerPoly);
            ...
            int npolys = 0;
            Extensions.ArrayFill(polys, DT_TILECACHE_NULL_IDX, 0, ntris * maxVertsPerPoly);
            ...
            for (int i = 0; i < npolys; ++i)
            {
                ...
                Extensions.ArrayFill(mesh.polys, DT_TILECACHE_NULL_IDX, p, maxVertsPerPoly * 2);
                for (int j = 0; j < maxVertsPerPoly; ++j)
        ...
        public DtTileCachePolyMesh BuildTileCachePolyMesh(DtTileCacheContourSet lcset, int maxVertsPerPoly)
        {
            ...
            Extensions.ArrayFill(mesh.polys, DT_TILECACHE_NULL_IDX);
            ...
            for (int i = 0; i < lcset.nconts; ++i)
            {
                ...
                // Build initial polygons.
                int npolys = 0;
                Extensions.ArrayFill(polys, DT_TILECACHE_NULL_IDX);
                for (int j = 0; j < ntris; ++j)
                {
                    int t = j * 3;
                    if (tris[t] != tris[t + 1] && tris[t] != tris[t + 2] && tris[t + 1] != tris[t + 2])
        ...
        public void MarkCylinderArea(DtTileCacheLayer layer, RcVec3f orig, float cs, float ch, RcVec3f pos, float radius, float height, int areaId)
        {
            ...
            int minx = (int)(float)Math.Floor((bmin.X - orig.X) * ics);
            int miny = (int)(float)Math.Floor((bmin.Y - orig.Y) * ich);
            int minz = (int)(float)Math.Floor((bmin.Z - orig.Z) * ics);
            int maxx = (int)(float)Math.Floor((bmax.X - orig.X) * ics);
            int maxy = (int)(float)Math.Floor((bmax.Y - orig.Y) * ich);
            int maxz = (int)(float)Math.Floor((bmax.Z - orig.Z) * ics);
            ...
            float[] rotAux, int areaId)
        {
            ...
            int minx = (int)(float)Math.Floor(cx - maxr * ics);
            int maxx = (int)(float)Math.Floor(cx + maxr * ics);
            int minz = (int)(float)Math.Floor(cz - maxr * ics);
            int maxz = (int)(float)Math.Floor(cz + maxr * ics);
            int miny = (int)(float)Math.Floor((center.Y - extents.Y - orig.Y) * ich);
            int maxy = (int)(float)Math.Floor((center.Y + extents.Y - orig.Y) * ich);
