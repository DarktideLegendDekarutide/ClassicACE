...
namespace ACE.Server.DotRecast.Recast
{
    public static class RcRasterizations
    {
        ...
        public static void AddSpan(RcHeightfield heightfield, int x, int z, int min, int max, int areaID, int flagMergeThreshold)
        {
            ...
            while (currentSpan != null)
            {
                ...
                else
                {
                    ...
                    // Merge flags.
                    if ((float)Math.Abs(newSpan.smax - currentSpan.smax) <= flagMergeThreshold)
                    {
            ...
            int flagMergeThreshold)
        {
            ...
            z0 = Extensions.MathClamp(z0, -1, h - 1);
            z1 = Extensions.MathClamp(z1, 0, h - 1);
            ...
            for (int z = z0; z <= z1; ++z)
            {
                ...
                x0 = Extensions.MathClamp(x0, -1, w - 1);
                x1 = Extensions.MathClamp(x1, 0, w - 1);
                ...
                for (int x = x0; x <= x1; ++x)
                {
                    ...
                    int spanMinCellIndex = Extensions.MathClamp((int)(float)Math.Floor(spanMin * inverseCellHeight), 0, RC_SPAN_MAX_HEIGHT);
                    int spanMaxCellIndex = Extensions.MathClamp((int)(float)Math.Ceiling(spanMax * inverseCellHeight), spanMinCellIndex + 1, RC_SPAN_MAX_HEIGHT);
