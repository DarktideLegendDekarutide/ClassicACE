...
namespace ACE.Server.DotRecast.Recast
{
    ...
    public static class RcMeshDetails
    {
        ...
        private static int GetHeight(float fx, float fy, float fz, float cs, float ics, float ch, int radius,
            RcHeightPatch hp)
        {
            int ix = (int)(float)Math.Floor(fx * ics + 0.01f);
            int iz = (int)(float)Math.Floor(fz * ics + 0.01f);
            ix = Extensions.MathClamp(ix - hp.xmin, 0, hp.width - 1);
            iz = Extensions.MathClamp(iz - hp.ymin, 0, hp.height - 1);
            ...
            int heightSearchRadius, RcCompactHeightfield chf, RcHeightPatch hp, float[] verts, List<int> tris)
        {
            ...
            if (sampleDist > 0)
            {
                for (int i = 0, j = nin - 1; i < nin; j = i++)
                {
                    ...
                    if ((float)Math.Abs(@in[vj + 0] - @in[vi + 0]) < 1e-6f)
                    {
                        if (@in[vj + 2] > @in[vi + 2])
                        {
                            (vi, vj) = (vj, vi);
                    ...
                    int nn = 1 + (int)(float)Math.Floor(d / sampleDist);
            ...
            if (sampleDist > 0)
            {
                ...
                int x0 = (int)(float)Math.Floor(bmin.X / sampleDist);
                int x1 = (int)(float)Math.Ceiling(bmax.X / sampleDist);
                int z0 = (int)(float)Math.Floor(bmin.Z / sampleDist);
                int z1 = (int)(float)Math.Ceiling(bmax.Z / sampleDist);
                samples.Clear();
                for (int z = z0; z < z1; ++z)
                {
                    for (int x = x0; x < x1; ++x)
            ...
            int[] verts, int bs, RcHeightPatch hp, List<int> array)
        {
            ...
            int[] dirs = { 0, 1, 2, 3 };
            Extensions.ArrayFill(hp.data, 0, 0, (hp.width * hp.height) - (0));
            ...
            array.Add(cy + bs);
            array.Add(ci);
            Extensions.ArrayFill(hp.data, RC_UNSET_HEIGHT, 0, (hp.width * hp.height) - (0));
            ...
            float sampleDist, float sampleMaxError)
        {
            ...
            int heightSearchRadius = (int)Math.Max(1, (float)Math.Ceiling(mesh.maxEdgeError));
