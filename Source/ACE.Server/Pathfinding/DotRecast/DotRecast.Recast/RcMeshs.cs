...
namespace ACE.Server.DotRecast.Recast
{
    ...
    public static class RcMeshs
    {
        ...
        private static int AddVertex(int x, int y, int z, int[] verts, int[] firstVert, int[] nextVert, ref int nv)
        {
            ...
            {
                int v = i * 3;
                if (verts[v + 0] == x && ((float)Math.Abs(verts[v + 1] - y) <= 2) && verts[v + 2] == z)
                    return i;
                i = nextVert[i]; // next
            }
        ...
        private static void MergePolyVerts(int[] polys, int pa, int pb, int ea, int eb, int tmp, int nvp)
        {
            ...
            // Merge polygons.
            Extensions.ArrayFill(polys, RC_MESH_NULL_IDX, tmp, (tmp + nvp) - (tmp));
            int n = 0;
            // Add pa
        ...
        private static void RemoveVertex(RcContext ctx, RcPolyMesh mesh, int rem, int maxTris)
        {
            ...
            for (int i = 0; i < mesh.npolys; ++i)
            {
                ...
                if (hasRem)
                {
                    ...
                    Extensions.ArrayFill(mesh.polys, RC_MESH_NULL_IDX, p + nvp, (p + nvp + nvp) - (p + nvp));
            ...
            int npolys = 0;
            Extensions.ArrayFill(polys, RC_MESH_NULL_IDX, 0, (ntris * nvp) - (0));
            ...
            for (int i = 0; i < npolys; ++i)
            {
                ...
                Extensions.ArrayFill(mesh.polys, RC_MESH_NULL_IDX, p, (p + nvp * 2) - (p));
                for (int j = 0; j < nvp; ++j)
        ...
        public static RcPolyMesh BuildPolyMesh(RcContext ctx, RcContourSet cset, int nvp)
        {
            ...
            Extensions.ArrayFill(mesh.polys, RC_MESH_NULL_IDX);
            ...
            for (int i = 0; i < cset.conts.Count; ++i)
            {
                ...
                // Build initial polygons.
                int npolys = 0;
                Extensions.ArrayFill(polys, RC_MESH_NULL_IDX);
                for (int j = 0; j < ntris; ++j)
                {
                    int t = j * 3;
                    if (tris[t + 0] != tris[t + 1] && tris[t + 0] != tris[t + 2] && tris[t + 1] != tris[t + 2])
        ...
        public static RcPolyMesh MergePolyMeshes(RcContext ctx, RcPolyMesh[] meshes, int nmeshes)
        {
            ...
            Extensions.ArrayFill(mesh.polys, RC_MESH_NULL_IDX, 0, (mesh.polys.Length) - (0));
            ...
            for (int i = 0; i < nmeshes; ++i)
            {
                ...
                int ox = (int)(float)Math.Floor((pmesh.bmin.X - mesh.bmin.X) / mesh.cs + 0.5f);
                int oz = (int)(float)Math.Floor((pmesh.bmin.Z - mesh.bmin.Z) / mesh.cs + 0.5f);

                bool isMinX = (ox == 0);
                bool isMinZ = (oz == 0);
                bool isMaxX = ((float)Math.Floor((mesh.bmax.X - pmesh.bmax.X) / mesh.cs + 0.5f)) == 0;
                bool isMaxZ = ((float)Math.Floor((mesh.bmax.Z - pmesh.bmax.Z) / mesh.cs + 0.5f)) == 0;
