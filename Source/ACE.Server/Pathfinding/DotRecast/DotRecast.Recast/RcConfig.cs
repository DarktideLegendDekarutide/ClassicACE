...
namespace ACE.Server.DotRecast.Recast
{
    ...
    public class RcConfig
    {
            ...
            RcAreaModification walkableAreaMod, bool buildMeshDetail)
        {
            ...
            WalkableHeight = (int)(float)Math.Ceiling(agentHeight / Ch);
            WalkableHeightWorld = agentHeight;
            WalkableClimb = (int)(float)Math.Floor(agentMaxClimb / Ch);
            WalkableClimbWorld = agentMaxClimb;
            WalkableRadius = (int)(float)Math.Ceiling(agentRadius / Cs);
            WalkableRadiusWorld = agentRadius;
            MinRegionArea = (int)(float)Math.Round(minRegionArea / (Cs * Cs));
            MinRegionAreaWorld = minRegionArea;
            MergeRegionArea = (int)(float)Math.Round(mergeRegionArea / (Cs * Cs));
        ...
        {
            return 3 + (int)(float)Math.Ceiling(agentRadius / cs);
        }
    }
}