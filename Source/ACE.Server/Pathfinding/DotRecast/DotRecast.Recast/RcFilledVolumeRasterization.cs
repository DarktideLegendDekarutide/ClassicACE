...
using static ACE.Server.DotRecast.Recast.RcConstants;
...
namespace ACE.Server.DotRecast.Recast
{
    public static class RcFilledVolumeRasterization
    {
            ...
            Func<float[], float[]> intersection)
        {
            ...
            for (int x = xMin; x <= xMax; x++)
            {
                for (int z = zMin; z <= zMax; z++)
                {
                    ...
                    if (h != null)
                    {
                        int smin = (int)(float)Math.Floor((h[0] - hf.bmin.Y) * ich);
                        int smax = (int)(float)Math.Ceiling((h[1] - hf.bmin.Y) * ich);
                        if (smin != smax)
                        {
                            int ismin = Extensions.MathClamp(smin, 0, RC_SPAN_MAX_HEIGHT);
                            int ismax = Extensions.MathClamp(smax, ismin + 1, RC_SPAN_MAX_HEIGHT);
        ...
        private static float[] IntersectCylinder(float[] rectangle, RcVec3f start, RcVec3f end, RcVec3f axis, float radiusSqr)
        {
            float[] s = MergeIntersections(
                RayCylinderIntersection(new RcVec3f(
                    Extensions.MathClamp(start.X, rectangle[0], rectangle[2]), rectangle[4],
                    Extensions.MathClamp(start.Z, rectangle[1], rectangle[3])
                ), start, axis, radiusSqr),
                RayCylinderIntersection(new RcVec3f(
                    Extensions.MathClamp(end.X, rectangle[0], rectangle[2]), rectangle[4],
                    Extensions.MathClamp(end.Z, rectangle[1], rectangle[3])
                ), start, axis, radiusSqr));
            float axisLen2dSqr = axis.X * axis.X + axis.Z * axis.Z;
            if (axisLen2dSqr > EPSILON)
            {
        ...
        private static RcVec3f XSlabRayIntersection(float[] rectangle, RcVec3f start, RcVec3f direction, float x)
        {
            ...
            float z = Extensions.MathClamp(start.Z + t * direction.Z, rectangle[1], rectangle[3]);
        ...
        private static RcVec3f ZSlabRayIntersection(float[] rectangle, RcVec3f start, RcVec3f direction, float z)
        {
            ...
            float x = Extensions.MathClamp(start.X + t * direction.X, rectangle[0], rectangle[2]);
            return new RcVec3f(x, rectangle[4], z);
        }
        ...
        private static float[] IntersectBox(float[] rectangle, float[] vertices, float[][] planes)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    if ((float)Math.Abs(planes[j][1]) > EPSILON)
                    {
            ...
            float[][] triBounds)
        {
            ...
            for (int tr = 0, tri = 0; tri < triangles.Length; tr++, tri += 3)
            {
                ...
                if ((float)Math.Abs(planes[tri][1]) < EPSILON)
                {
                    continue;
                }
